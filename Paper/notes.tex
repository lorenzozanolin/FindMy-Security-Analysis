...
Each BLE beacon contains an Elliptic Curve public key $p_i$. When a finder device receives a beacon, it saves its location and encrypts it using the $p_i$ key; Subsequently, it uploads the packet to Apple's servers.
OF uses the Elliptic Curve Integrated Encryption Scheme (ECIES), which generates a shared secret that is utilized to encrypt the report through an ephemeral Elliptic Curve Diffie-Hellmann (ECDH) key exchange. According to \cite{whocanfind}, ``the finder’s encryption algorithm works as follows:
\begin{enumerate}
