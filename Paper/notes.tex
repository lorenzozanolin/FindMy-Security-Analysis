...
\section{Conclusion}
In conclusione, nella sezione 1 è stata data una overview generale delle tematiche che il paper avrebbe indagato. Nella sezione 2 è stata presentata la tecnologia UWB in cui, dopo una overview generale, è stata fatta un'analisi della sicurezza concentrandosi nello specifico sulle tecniche adottate da Apple nell'utilizzo di questa tecnologia. 
Continuando, nella sezione 3 si è passati ad approfondire la rete FindMy; si è iniziato con una overview generale, in cui sono state descritte le features offerte da questo servizio, e successivamente è stato descritto un esempio di funzionamento. Di seguito, è stata fatta una dettagliata analisi alla sicurezza

Successivamente  possiamo affermare che questi dispositivi offrono un'ottima funzionalità per il basso costo a cui è possibile acquistarli ma sono ancora troppo poco sicuri; una miglioria potrebbe essere introdurre un sistema di autenticazione nella fase in cui vengono scambiate le chiavi pubbliche, per evitare attacchi replay come descritto in \ref{sec}.
